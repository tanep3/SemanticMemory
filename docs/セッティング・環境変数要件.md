...
| キー                        | 用途               | 初期値                                                    |
| ------------------------- | ---------------- | ------------------------------------------------------ |
| `sbert_model`             | 埋め込みモデル          | `cl-nagoya/ruri-v3-70m`                               |
| `cosine_threshold`        | コサイン類似度閾値        | `0.5` ✅                                                |
...
* `sbert_model`以外は**Web UI/APIから変更可能**
* `sbert_model`はベクトル再構築を伴うため`/api/rebuild_vector`からのみ変更可能
* 初期値は**DB初期化スクリプトでセット**
* 新しい既定値は新規DBだけに適用し、既存DBの保存値は上書きしない
